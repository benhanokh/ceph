  ...
  struct dedup_filter_t {
    ...
    bool should_process_bucket(const std::string& bucket_name) const {
      if (!allow_buckets.empty())
        return allow_buckets.count(bucket_name) > 0;
      if (!deny_buckets.empty())
        return deny_buckets.count(bucket_name) == 0;
      return true;
    }

    bool should_process_storage_class(const std::string& storage_class) const {
      if (!allow_storage_classes.empty())
        return allow_storage_classes.count(storage_class) > 0;
      if (!deny_storage_classes.empty())
        return deny_storage_classes.count(storage_class) == 0;
      return true;
    }
